...
package com.ivianuu.materialdialogs.callback

import com.ivianuu.materialdialogs.DialogCallback
 ...
 */
fun MaterialDialog.onShow(callback: DialogCallback) = apply {
    if (!showListeners.contains(callback)) {
        showListeners.add(callback)
    }
    if (isShowing) {
        // Already showing, invoke now
        showListeners.invokeAll(this)
    }
}
 ...
 * multiple times, it appends additional callbacks, rather than overwriting.
 */
fun MaterialDialog.onDismiss(callback: DialogCallback) = apply {
    if (!dismissListeners.contains(callback)) {
 ...
 * multiple times, it appends additional callbacks, rather than overwriting.
 */
fun MaterialDialog.onCancel(callback: DialogCallback) = apply {
    if (!cancelListeners.contains(callback)) {
